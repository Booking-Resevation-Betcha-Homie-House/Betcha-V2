document.addEventListener('DOMContentLoaded', function () {
  ...
  function createPropertyCard(property) {
    return `
      <a href="view-property.html" class="property-card">
        <div class="room-list">
          <div class="bg-neutral-300 w-full aspect-square rounded-3xl mb-3" 
               style="background-image: url('${property.photoLinks[0]}'); 
                      background-size: cover; 
                      background-position: center;">
          </div>
          <div class="flex flex-col items-start mx-3">
            <div class="flex justify-between items-center w-full">
              <p class="font-inter text-primary-text">${property.name}</p>
  ...
  async function fetchAndDisplayProperties(category, containerId) {
    try {
      ...
      if (container && data[category]) {
        container.innerHTML = data[category].map(property => createPropertyCard(property)).join('');
      }
    } catch (error) {
  ...
  window.setActiveTab = async function (index) {
    ...
    const containers = [
      ...
      { id: 'coupleContent', category: 'couple' },
    ...
    containers.forEach((container, i) => {
      ...
      if (content) {
        if (i === index) {
          ...
          fetchAndDisplayProperties(container.category, container.id);
        } else {
